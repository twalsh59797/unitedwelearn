...
from better_mistakes.model.losses import HierarchicalCrossEntropyLoss, OurHierarchicalCrossEntropyLoss, CosineLoss, RankingLoss, CosinePlusXentLoss, YOLOLoss
...
def main(test_opts):
    ...
    for experiment in experiment_to_epochs.keys():
        ...
        expm_json_path = os.path.join(test_opts.experiments_path, experiment.split("/")[-1], "opts.json")
        ...
        if opts.loss == "cross-entropy":
            ...
            loss_function = OurHierarchicalCrossEntropyLoss(hierarchy, classes, weights=opts.weights,
                                                            return_all=opts.all_levels).cuda(opts.gpu)
        ...
        for e in epochs_to_run:
            ...
            checkpoint_path = os.path.join(test_opts.experiments_path, experiment.split("/")[-1], "model_snapshots", checkpoint_id)
        ...
        with open(os.path.join(test_opts.experiments_path, experiment.split("/")[-1], "test_summary.json"), "w") as fp:
            json.dump(summaries, fp, indent=4)
        with open(os.path.join(test_opts.experiments_path, experiment.split("/")[-1], "test_summary_table.json"), "w") as fp:
