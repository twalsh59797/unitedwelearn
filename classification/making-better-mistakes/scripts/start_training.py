...
from better_mistakes.model.losses import OurHierarchicalCrossEntropyLoss, HierarchicalCrossEntropyLoss, CosineLoss, RankingLoss, CosinePlusXentLoss, YOLOLoss
from better_mistakes.trees import load_hierarchy, get_weighting, load_distances, get_classes

MODEL_NAMES = sorted(name for name in models.__dict__ if name.islower() and not name.startswith("__") and callable(models.__dict__[name]))
LOSS_NAMES = ["cross-entropy", "soft-labels", "hierarchical-cross-entropy", "cosine-distance", "ranking-loss", "cosine-plus-xent", "yolo-v2", "our-hierarchical-cross-entropy"]
...
def main_worker(gpus_per_node, opts):
    ...
    elif opts.loss == "our-hierarchical-cross-entropy":
        loss_function = OurHierarchicalCrossEntropyLoss(hierarchy, classes, weights=opts.weights).cuda(opts.gpu)
    ...
    if opts.multistep_lr:
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=opts.lr_steps, gamma=0.1)
    for epoch in range(opts.start_epoch, opts.epochs):
        ...
        if opts.devise or opts.barzdenzler:
            ...
            if opts.multistep_lr:
                ...
                print("scheduler", scheduler, opts.lr_steps, type(opts.lr_steps), scheduler.get_lr())
...
if __name__ == "__main__":
    ...
    # Our/HXE --------------------------------------------------------------------------------------
    parser.add_argument("--weights", type=float, nargs="+", default=None, help="Weights for hierarchical cross entropy.")
    parser.add_argument("--multistep_lr", type=boolean, default=False, help="use multi step learning rate")
    parser.add_argument("--lr_steps", type=int, nargs="+", default=[35], help="Steps for learning rate decrease.")
    # Devise/B&D ----------------------------------------------------------------------------------------------------------------------------------------------
