...
class OurHierarchicalCrossEntropyLoss(torch.nn.Module):
    """
    Our implementation of hierarchical XE
    """

    def __init__(self, hierarchy: Tree, classes: List[str], weights: Optional[Tuple] = None):
        super(OurHierarchicalCrossEntropyLoss, self).__init__()
        assert set(classes) == set(hierarchy.leaves())

        level_dicts = []
        level_hierarchy = hierarchy

        assert hierarchy.height() == 8
        ...
        self.hierarchical_matrices = hierarchical_matrices

        self.weights = (1./(len(self.hierarchical_matrices) + 1.),) * (len(self.hierarchical_matrices) + 1)
        if weights:
            self.weights = weights

        assert len(self.hierarchical_matrices) == len(self.weights) - 1

    def forward(self, inputs, target):
        # return torch.nn.functional.cross_entropy(inputs, target)
        total_loss = 0
        for level_i in range(len(self.hierarchical_matrices) + 1):
            if level_i == 0:
                level_inputs, level_target = inputs, target
            else:
                level_transform = torch.from_numpy(self.hierarchical_matrices[level_i - 1]).float()
                level_transform = level_transform.cuda()
                level_inputs = torch.matmul(prev_level_inputs, level_transform)
                mapping = np.argmax(self.hierarchical_matrices[level_i - 1], axis=1)
            ...
            total_loss += loss_level * self.weights[level_i]
